...
func TestRunAutomation_Success(t *testing.T) {
	...
	err := runAutomation(root, "hello", nil, nil, os.Stdout, os.Stderr)
	if err != nil {
...
func TestRunAutomation_NestedName(t *testing.T) {
	root := setupRunWorkspace(t)
	err := runAutomation(root, "docker/up", nil, nil, os.Stdout, os.Stderr)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestRunAutomation_NotFound(t *testing.T) {
	root := setupRunWorkspace(t)
	err := runAutomation(root, "nonexistent", nil, nil, os.Stdout, os.Stderr)
	if err == nil {
...
func TestRunAutomation_ExitCode(t *testing.T) {
	root := setupRunWorkspace(t)
	err := runAutomation(root, "fail", nil, nil, os.Stdout, os.Stderr)
	if err == nil {
...
func TestRunAutomation_WithArgs(t *testing.T) {
	root := setupRunWorkspace(t)
	err := runAutomation(root, "args", []string{"foo", "bar"}, nil, os.Stdout, os.Stderr)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestRunAutomation_RunStep(t *testing.T) {
	root := setupRunWorkspace(t)
	err := runAutomation(root, "chain", nil, nil, os.Stdout, os.Stderr)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestRunAutomation_FromSubdirectory(t *testing.T) {
	...
	err := runAutomation(sub, "hello", nil, nil, os.Stdout, os.Stderr)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestRunAutomation_NoPiYaml(t *testing.T) {
	dir := t.TempDir()
	err := runAutomation(dir, "hello", nil, nil, os.Stdout, os.Stderr)
	if err == nil {
...
`), 0o644)
	...
	err := runAutomation(root, "greet", nil, map[string]string{"name": "alice"}, &buf, os.Stderr)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
