...
internal/
  ...
  executor/                        Step execution engine
    executor.go                    Executor struct (with ParentEvalFile, Runners, cachedRegistry, Outputs (interpolation.OutputTracker), condEval, DryRun fields), stepExecCtx (bundles per-automation execution state: automation, args, inputEnv, automationEnv — constant across all steps in a RunWithInputs call), ExitError, Run(), RunWithInputs(), execStep(displayStdout, stderrW) — unified step dispatch with explicit I/O routing (no Executor state mutation for suppression), execParentShell(), execFirstBlock(), AppendToParentEval(), evaluateCondition() (delegates to conditions.Evaluator), conditionEvaluator() (lazy-init), pushCall()/popCall(), printer(), registry() (lazy-init with caching), newRunContext(workDir, stderrW) — caller resolves dir: and stderr once and passes result, stdout()/stderr()/stdin(); interpolation delegated to internal/interpolation package (interpolateWithCtx/interpolateWith/interpolateEnv/interpolateValue are thin wrappers for backward compatibility); DryRun mode delegates to dryRunAutomation(); GoFunc automation dispatch; pipe: true orchestration; outputs.last inter-step output capture (via io.MultiWriter + interpolation.OutputTracker); step-level and automation-level if: conditional execution; automation-level env merged per step (not passed into run: sub-automations); env: values support outputs.last/outputs.<N>/inputs.<name> interpolation (step-level resolved per step, automation-level resolved once at start); step-level silent: true suppression via io.Discard (callers pass io.Discard for displayStdout/stderrW); --loud override; parent_shell: true eval-file delegation; step dispatch via Registry; dir: resolved once per step in execStep; first: block first-match dispatch; PI_ARGS env var injection for extra args on automations without inputs; step error messages include automation name context
