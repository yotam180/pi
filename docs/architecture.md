...
internal/
  ...
  executor/                        Step execution engine
    ...
    runner_iface.go                StepRunner interface, RunContext (step execution context with WorkDir, RuntimePaths, InterpolateWith callback — no BuildEnv callback; env construction uses standalone BuildStepEnv), Registry (maps StepType→StepRunner), NewRegistry(), NewDefaultRegistry(), FileExtForStepType(), DefaultFileExtensions()
    runners.go                     Step runner implementations: SubprocessRunner (config-driven, shared by bash/python/typescript), SubprocessConfig (Binary/BinaryFunc, InlineArgs, TempFileExt, FileExt, NotFoundMsg, Language), NewBashRunner(), NewPythonRunner(), NewTypeScriptRunner(), RunStepRunner; runStepCommand() shared command execution with timeout support (exec.CommandContext), calls BuildStepEnv(ctx.RuntimePaths, ...) directly; TimeoutExitCode (124); writeTempScript(), resolvePythonBin(), isCommandNotFound()
...
- `SubprocessRunner` in `runners.go` is a config-driven runner that handles the full subprocess lifecycle: file-path resolution → inline-script handling (via `InlineArgs` or temp-file) → command execution → error wrapping. `SubprocessConfig` captures per-language differences (binary, inline arg format, temp file extension, file detection extension, not-found message). All language-specific runners (`NewBashRunner`, `NewPythonRunner`, `NewTypeScriptRunner`) are thin config constructors. Adding a new language runner requires only a ~10-line config struct — no new types needed. `FileExt` on each runner determines which step values are treated as file references (e.g. `.sh` for bash, `.py` for python) — no global hardcoded extension list.
