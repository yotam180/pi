...
- `validate` provides a registry-based validation system with `Check` interface, `Runner`, and `DefaultRunner()`; depends on `automation`, `config`, `discovery`, and `conditions` (no executor dependency — file-path detection and step metadata come from `automation.IsFilePath`, `automation.DefaultFileExtensions`, and `automation.StepTypeSupportsParentShell`)
