...
internal/
  ...
  cli/                             Cobra CLI commands
    ...
    package_fetcher.go             PackageFetcher interface (Fetch → path, wasCached, error), CachePackageFetcher (wraps cache.Cache), NewCachePackageFetcher(); decouples CLI from cache package for testability and extensibility
    discover.go                    discoverAll() / discoverAllWithConfig() — discovers local + package + built-in automations and merges; fetches/verifies packages from config via PackageFetcher; passes caller-provided stderr for all warnings (discovery, merge, on-demand fetch); newOnDemandFetcher(stderr, fetcher) — creates on-demand GitHub package fetch callback with per-invocation dedup and advisory output; printOnDemandAdvisory() — prints fetch status and pi.yaml snippet to stderr; resolveGitHubPackage() accepts PackageFetcher for testable GitHub resolution
    ...
    package_fetcher_test.go        22 tests — mockFetcher-based tests: resolveGitHubPackage [cache hit, fresh fetch, fetch error, invalid source, nil printer], fetchGitHubPackage [cached, fresh, no version, fetch error], newOnDemandFetcher [fetch and merge with advisory, cached hit no advisory, dedup, fetch error, empty path, automation not found, nil stderr], mergePackages [GitHub package, GitHub fetch error], runAddWithFetcher [GitHub cached, GitHub fetch error, GitHub with alias, file source ignores fetcher]
    run.go                         pi run — resolves and executes automations; SetInterspersed(false) stops flag parsing at automation name so post-name args forward naturally (no -- needed); --repo flag; --with key=value flag; --silent flag; --loud flag; --dry-run flag (shows steps without executing); all pi flags must come before automation name; uses ProjectContext for resolution and executor construction
    ...
    doctor.go                      pi doctor — scans all automations, checks requires: entries, prints health table; color-coded ✓/✗ via display.Printer; uses reqcheck.CheckRequirementForDoctor() and reqcheck.InstallHintFor() directly (no executor dependency for requirement checking)
    validate.go                    pi validate — thin CLI wrapper over internal/validate package; resolves project, builds validate.Context, runs validate.DefaultRunner(), prints errors/success; --warnings/-w flag enables non-fatal warning checks; exit 0/1
    add.go                         pi add — validates source via refparser.Parse(); fetches GitHub packages via PackageFetcher; calls config.AddPackage() to update pi.yaml; --as for aliases; idempotent (duplicate sources print "already in pi.yaml" and exit successfully); runAddWithFetcher() accepts PackageFetcher for testability
    ...
    discover_test.go               20 tests (advisory output format, nil writer, fetch status text, down arrow icon, discoverAllWithConfig [warnings route to stderr, nil stderr suppresses], discoverAll nil stderr, mergePackages [shadow warnings route to stderr, nil stderr suppresses], resolveFilePackage [existing dir, alias, missing, missing alias, not-a-dir, relative path, nil printer, nil stderr], resolvePackageSource file routing, mergePackages [empty list, missing file source skipped])
    run_test.go                    pi run tests (26 tests — includes --with, inputs, --silent, --dry-run, excess positional args, PI_ARGS, natural arg forwarding, Cobra flag isolation tests)
    validate_test.go               pi validate tests (81 tests — integration tests via runValidate(); unit tests now delegate to validate.CheckWithInputs, validate.DetectCycles, validate.NormalizeCycleKey; covers valid project, broken refs, multiple errors, builtin refs, no pi.yaml, broken file refs, valid file refs, inline scripts, first: block file refs, subdir file refs, installer file refs, shortcut/setup/run-step with: validation, circular deps, condition validation, unknown field detection with suggestions, pi.yaml unknown field detection with suggestions, --warnings flag [show/hide, clean project, with errors, multiple types])
    add_test.go                    pi add tests (8 tests — file source, file with alias, idempotent duplicate, no version error, invalid source, no pi.yaml, no args, builtin ref error); see also package_fetcher_test.go for GitHub fetch tests via mock
  ...
  config/                          pi.yaml parsing
    ...
    writer.go                      AddPackage() — reads pi.yaml, detects duplicates (DuplicatePackageError), delegates to insertIntoBlock(); AddSetupEntry() — reads pi.yaml, finds matching entries via findMatchingEntry() (exact duplicate → DuplicateSetupEntryError, same run+if with different with → replaces in-place via replaceSetupEntry() → ReplacedSetupEntryError, no match → delegates to insertIntoBlock()); FormatSetupEntry() renders setup entries as YAML; shared YAML block helpers: findBlockIndex(), appendToBlock(), appendNewBlock(), insertIntoBlock() — generic block insertion/creation used by both packages and setup writers; line-based raw string manipulation preserves unrelated file content
    writer_test.go                 48 tests (14 package + 25 setup + 9 shared helpers: GitHub/file add, alias, duplicate, replace bare→versioned, replace versioned→bare, replace preserves position, replace multi-line, ReplacedSetupEntryError message, append, create block, missing pi.yaml, preserve content, multiple adds, format entry, insert entry, duplicate error, existing block with following content, findBlockIndex found/not-found/multiple, appendToBlock simple/trailing-blanks, appendNewBlock basic/strip-newlines, insertIntoBlock existing/no-block)
  automation/                      Individual automation YAML parsing
    ...
    step.go                        StepType, Step (with If, Env, Silent, ParentShell, Dir, Timeout, Description, First, Pipe), stepRaw (YAML pipe + pipe_to), resolvePipe(index, warnWriter), toStep(index, warnWriter), toFirstStep(index, warnWriter), IsFirst(), InstallPhase, InstallSpec, validateSingleStep() — shared helper for single-step invariants (empty value, valid type, parseable if:), validateSteps(), validateFirstBlock(), validateInstall(), validateInstallPhase(); all three validation loops delegate to validateSingleStep() to eliminate duplication
    ...
    requirements.go                RequirementKind, Requirement, requirementRaw, parseNameVersion(), validateVersionString(); knownRuntimes derived from runtimeinfo.KnownNames()
    step_metadata.go               IsFilePath(value, ext) — pure string check for file-path vs inline-script detection; DefaultFileExtensions() — static map of step type → file extension (bash→.sh, python→.py, typescript→.ts); StepTypeSupportsParentShell(stepType) — static parent_shell capability per step type; these are compile-time metadata used by validate without importing executor
    ...
    step_test.go                   91 tests (if/env/silent/parent_shell/dir/timeout/description/pipe fields, install block, first: block, first: block error paths [timeout/silent/parent_shell/with on block, sub-step invalid if/empty value, outer invalid if], install phase error paths [empty step-list, verify invalid if, run empty step, run first-block invalid if], validateSingleStep unit tests)
    walker_test.go                 25 tests (regular steps, first: blocks, installer scalar/step-list, verify absent, early stop, FormatPath, field preservation, WalkStepsUntil stops in run/verify/test phases, step-list phase with first: block, completes all install phases, no install)
    ...
    step_metadata_test.go          4 tests (IsFilePath, DefaultFileExtensions copy safety, StepTypeSupportsParentShell, DefaultFileExtensionsConsistency)
  display/                         Styled terminal output (color, TTY detection)
    display.go                     StatusKind type (StatusSuccess/StatusSuccessCached/StatusInProgress/StatusFailed/StatusWarning), statusIcon(), printStatusLine(); Printer struct, color methods (Plain, Dim, Green, Red, Bold, Warn), InstallStatus(StatusKind, ...), SetupHeader, StepTrace, PackageFetch(StatusKind, ...), truncateTrace, shouldColor; NewForWriter() — auto-detects TTY for arbitrary io.Writer (used by CLI commands that receive io.Writer stderr)
    tty.go                         isTerminal() via golang.org/x/term
    display_test.go                49 tests (styles, color toggle, NO_COLOR, TTY, install status variants, step trace, truncateTrace, package fetch status [all StatusKind variants], statusIcon unit tests, printStatusLine unit tests, Warn style, NewForWriter)
  discovery/                       .pi/ folder scanning and automation lookup
    ...
    suggest.go                     suggestNames() — thin wrapper over internal/suggest.TopN with discovery-specific threshold (30% of query length, min 3); used by findLocal() and findBuiltin() for "Did you mean?" error messages
    suggest_test.go                11 tests (suggestion sorting, max results, exact match exclusion, empty inputs, integration with Find)
  ...
  interpolation/                   Output tracking and value interpolation for step outputs, inputs, and env references
    interpolation.go               OutputTracker (Record, Last, Get, Len, Snapshot, Restore, Reset), ResolveValue (outputs.last/outputs.<N>/inputs.<name>), ResolveEnv (apply interpolation to env maps), ResolveWith (apply interpolation to with: maps); pure functions and value types — no I/O, no Executor dependency
    interpolation_test.go          36 tests (100% coverage) — OutputTracker lifecycle (empty/record/trim/get/len/snapshot/restore/reset/copy-safety), ResolveValue (literal/outputs.last/indexed/nil-tracker/inputs/hyphen/equals), ResolveEnv (nil/empty/no-change/outputs/inputs/mixed/unchanged-passthrough), ResolveWith (nil/empty/literal/outputs/inputs/multi-key)
  executor/                        Step execution engine
    executor.go                    Executor struct (with ParentEvalFile, Runners, cachedRegistry, Outputs (interpolation.OutputTracker), condEval, DryRun fields), stepExecCtx (bundles per-automation execution state: automation, args, inputEnv, automationEnv — constant across all steps in a RunWithInputs call), ExitError, Run(), RunWithInputs() (delegates to runWithContext), runWithContext(ctx, a, args, withArgs, stdout, stdin) — internal entry point carrying explicit context.Context for cancellation/timeout and explicit stdout/stdin (no Executor state mutation for recursive calls), execStep(goCtx, sCtx, ...) — unified step dispatch with explicit I/O routing, execParentShell(), execFirstBlock(goCtx, ...), AppendToParentEval(), evaluateCondition() (delegates to conditions.Evaluator), conditionEvaluator() (lazy-init), pushCall()/popCall(), printer(), registry() (lazy-init with caching), newRunContext(goCtx, workDir, stderrW) — caller resolves dir: and stderr once and passes result, stdout()/stderr()/stdin(); RunAutomation closure uses runWithContext directly (no Stdout/Stdin mutation); all interpolation delegated to internal/interpolation package (no wrapper methods — tests call interpolation.Resolve* directly); DryRun mode delegates to dryRunAutomation(); GoFunc automation dispatch; pipe: true orchestration; outputs.last inter-step output capture (via io.MultiWriter + interpolation.OutputTracker); step-level and automation-level if: conditional execution; automation-level env merged per step (not passed into run: sub-automations); env: values support outputs.last/outputs.<N>/inputs.<name> interpolation (step-level resolved per step, automation-level resolved once at start); step-level silent: true suppression via io.Discard; --loud override; parent_shell: true eval-file delegation; step dispatch via Registry; dir: resolved once per step in execStep; first: block first-match dispatch; PI_ARGS env var injection for extra args on automations without inputs; step error messages include automation name context
    dry_run.go                     Dry-run mode: dryRunAutomation(), dryRunStep(), dryRunRunStep() (resolves and recurses), dryRunFirstBlock() (shows all sub-steps with match/skip/not-reached indicators), dryRunInstaller() (shows test→run→verify→version lifecycle), dryRunInstallPhase(), dryRunPrint() (indented output with annotations); triggered when Executor.DryRun is true; evaluates conditions and resolves inputs without executing commands
    dry_run_test.go                27 tests — basic bash, no execution, conditional skip/execute, run step recursion, multiple steps, first: block, installer lifecycle, step annotations (silent/pipe), go-func, automation-level if, circular dependency handling, dir+timeout annotations, dryRunTruncate, run step with inputs, run step args forwarding, run step targeting installer, run step targeting go-func, run step find error, installer explicit verify, installer no version, installer step-list phase, first: block run step recursion, parent_shell annotation, installer phase condition error, installer with automation env
    runner_iface.go                StepRunner interface, FileExtProvider interface (optional — runners declare owned file extension), ParentShellCapable interface (optional — runners declare parent_shell support), RunContext (step execution context with Ctx context.Context, WorkDir, RuntimePaths, InterpolateWith callback, RunAutomation callback (accepts context.Context), ResolvedAutomationEnv/ResolvedStepEnv override fields — env construction uses standalone BuildStepEnv with resolved env overrides), Registry (maps StepType→StepRunner), NewRegistry(), NewDefaultRegistry(), FileExtForStepType() (queries FileExtProvider interface), Registry.StepTypeSupportsParentShell() (queries ParentShellCapable interface)
    runners.go                     Step runner implementations: SubprocessRunner (config-driven, shared by bash/python/typescript; implements FileExtProvider and ParentShellCapable), SubprocessConfig (Binary/BinaryFunc, InlineArgs, TempFileExt, FileExt, NotFoundMsg, SupportsParentShell, Language), NewBashRunner() (SupportsParentShell: true), NewPythonRunner(), NewTypeScriptRunner(), RunStepRunner (supports timeout: on run: steps via context.WithTimeout — timeout exceeded returns ExitError{Code: 124}); runStepCommand() shared command execution with timeout support (exec.CommandContext from RunContext.Ctx), calls BuildStepEnv(ctx.RuntimePaths, ...) directly; TimeoutExitCode (124); writeTempScript(), resolvePythonBin(), isCommandNotFound()
    install.go                     Installer lifecycle: execInstall(), execInstallPhase(), execInstallPhaseLive(), execInstallPhaseWithStderr(), execInstallFirstBlock(), captureVersion(), printInstallStatus(display.StatusKind, ...); structured test→run→verify→version lifecycle; color-coded installer status via display.StatusKind and display.Printer; install phase steps dispatch through execStep() with io.Discard for stdout (suppressed) and configurable stderr; first: block support via execInstallFirstBlock() which also delegates to execStep(); run phase streams stderr live to terminal; automation-level env properly applied to all phases; execInstallPhaseWithStderr() is the unified implementation — execInstallPhase() and execInstallPhaseLive() are thin wrappers that pass io.Discard or e.stderr() respectively
    helpers.go                     Shared utilities: resolveFileStep(automationDir, value, lang, ext) (file-path resolution + existence check, uses automation.IsFilePath), resolveScriptPath(), BuildStepEnv(runtimePaths, inputEnv, automationEnv, stepEnv) — standalone function (no Executor dependency), prependPathInEnv(), resolveStepDir(), expandTraceVars(value, inputEnv, automationEnv, stepEnv); PI_IN_* + PI_INPUT_* + PI_ARGS env injection; provisioned runtime PATH injection; automation-level and step-level env: injection; step-level dir: resolution; trace variable expansion for display
    validate.go                    ValidateRequirements() (with provisioning fallback), tryProvision(); uses reqcheck.CheckResult, reqcheck.ValidationError, reqcheck.FormatValidationError directly — no re-exports
    ...
    outputs_test.go                30 tests — outputs.last: passed via with, multi-step chain, pipe capture, silent capture, indexed output, inputs interpolation, combined outputs+inputs, literal passthrough, GoFunc via run step, GoFunc failure, GoFunc condition skip, ResolveValue unit tests (via interpolation package: no-match/outputs.last/indexed/inputs), ResolveEnv unit tests (via interpolation package: nil/empty/no-change/outputs.last/indexed/inputs/mixed), env interpolation integration (outputs.last in step env, outputs indexed in step env, inputs in step env, inputs in automation env, outputs.last in automation env, literal passthrough, mixed outputs+literals)
    executor_test.go               19 tests — core execution: bash inline/file, run step chaining, circular deps, multi-step, working dir, mixed bash+run, exit error, call stack isolation
    ...
    step_timeout_test.go           13 tests — step-level timeout: no timeout runs normally, not exceeded, exceeded (killed with exit 124), stops execution chain, with pipe: true, with silent, skipped by condition, multiple steps only timed-out killed; run step timeout: exceeded (exit 124), not exceeded, stops chain, multi-step target, with with:
    parent_shell_test.go           9 tests — parent shell: writes to eval file, multiple steps append, mixed with normal, no eval file error, skipped by condition, AppendToParentEval, python step rejected (capability check), typescript step rejected, Registry.StepTypeSupportsParentShell
    coverage_gaps_test.go          57 tests — coverage gap closures: ResolveWith (via interpolation package: empty/map/outputs/literal/multi-key), stdout/stderr/stdin nil fallback and explicit, printer lazy creation and explicit, evaluateCondition parse errors and unknown predicates (unit + automation-level + step-level), AppendToParentEval error paths (invalid path, read-only), registry custom/cached/precedence, capability interfaces (custom runner with FileExtProvider + ParentShellCapable, plain runner without, partial implementation, unregistered type), first: block no-match pipe capture + parent shell sub-step + condition error, resolveScriptPath absolute/relative, unimplemented step type, resolveStepDir branches, isCommandNotFound, writeTempScript, resolvePythonBin ± venv, InstallHintFor known/unknown, FormatValidationError with/without hint + skip satisfied, formatRequirementLabel branches, runtimeCommand branches, detectVersion mocked (--version and version subcommand fallback), extractVersion patterns, compareVersions (various + parse error), BuildStepEnv (empty/runtimePaths only), stepExecCtx roundtrip
    validate_test.go               34 tests (version extraction via reqcheck, version comparison via reqcheck, requirement checking via reqcheck, validation integration, error formatting via reqcheck, install hints via reqcheck, CheckRequirementForDoctor, InstallHintFor, provisioning integration, prependPathInEnv, BuildStepEnv)
    predicates_test.go             5 smoke tests calling conditions package directly (ResolvePredicatesWithEnv, DefaultRuntimeEnv, ResolvePredicates integration, ValidatePredicateName, ValidateConditionExpr); comprehensive tests are in conditions/evaluator_test.go
  reqcheck/                        Runtime requirement checking (extracted from executor)
    reqcheck.go                    CheckResult, ValidationError, CheckRequirement(), CheckRequirementForDoctor(), checkImpl() (shared logic with alwaysDetectVersion flag); RuntimeCommand() — delegates to runtimeinfo.Binary(); DetectVersion(), detectVersionExec(), ExtractVersion(), CompareVersions(), parseVersionParts() — version detection and comparison utilities; FormatValidationError(), FormatRequirementLabel() — user-facing error formatting; InstallHintFor() — delegates to tools.InstallHintFor()
    reqcheck_test.go               42 tests (ExtractVersion patterns, CompareVersions arithmetic + invalid + edge cases, RuntimeCommand mappings, CheckRequirement [command found/not found, runtime found/not found, version satisfied/too low/undetectable, rust/go variants, command with version], CheckRequirementForDoctor [detects version without constraint, not found, with constraint], DetectVersion [mocked env, fallback to version subcommand, nil ExecOutput], FormatValidationError [full format, skips satisfied], FormatRequirementLabel [all label variants], InstallHintFor [known tools, unknown, rust family], ValidationError.Error(), parseVersionParts [valid/invalid])
  tools/                           Central tool registry — single source of truth for installable tool metadata
    tools.go                       ToolDescriptor (BuiltinName/ShortNames/InstallHint), Registry slice, BuildShortNameMap() (generates short-name-to-builtin map for setup add), InstallHintFor() (lookup by tool name for pi doctor), ToolResolutionHelp() (generates help text for pi setup add --help); entries with BuiltinName map to pi:install-* automations; entries without BuiltinName are command-only (docker, jq, git, etc.) for install hints
    tools_test.go                  11 tests (BuildShortNameMap all entries + pi: prefix variants + command-only exclusion, InstallHintFor known/unknown, ToolResolutionHelp contains all builtins + deterministic + prefers canonical, RegistryCoversAllInstallBuiltins, RegistryBuiltinNamesExist)
  ...
  runtimeinfo/                     Single source of truth for runtime identity
    runtimeinfo.go                 Descriptor (Name/Binary/DefaultVersion/DirectDownload/InstallHint), Runtimes slice, Find(), KnownNames(), SortedNames(), Binary(), DefaultVersion(); all runtime knowledge consolidated here — consumers import this instead of maintaining their own maps
    runtimeinfo_test.go            11 tests (Find known/unknown, KnownNames, SortedNames, Binary known/unknown, DefaultVersion known/unknown, AllHaveRequiredFields, NoDuplicateNames)
  runtimes/                        Sandboxed runtime provisioning
    runtimes.go                    CmdRunner interface (Run/Output) — abstracts command execution for testability; execCmdRunner — default os/exec implementation; KnownRuntimes derived from runtimeinfo.KnownNames(); Provisioner (with Runner field for CmdRunner injection), Provision(), provisionWithMise(), provisionDirect() (descriptor-gated), provisionNodeDirect(), provisionPythonDirect(), provisionGoDirect(), provisionRustDirect(), PrependToPath(); defaultVersion() and runtimeBinary() delegate to runtimeinfo
    runtimes_test.go               43 tests (provisioner defaults, config, never mode, unknown runtime, already provisioned, ask mode [nil prompt, user declines, user accepts, version in prompt, no version in prompt], binDirFor [not provisioned, provisioned, default version], prependToPath, defaultVersion, runtimeBinary, knownRuntimes, mise fallback, provisionDirect [unknown runtime, all known runtimes supported], unknown manager, binDir default version path, stderr default, mise integration, CmdRunner [defaults to exec, custom runner, execCmdRunner run/output/error], provisionWithMise mock [success, install failure, where failure, symlink overwrite, empty bin dir, nonexistent bin dir], direct provisioning mock [node runner called, python runner called, go runner called, go script URL, rust runner called, rust script version], auto mode default version, knownRuntimeList)
  ...
  suggest/                         Shared string-distance and suggestion utilities (zero dependencies on other PI packages)
    suggest.go                     Levenshtein() — Wagner–Fischer edit distance; TopN() — top N closest candidates sorted by distance then alphabetically; Best() — single closest match; used by discovery (suggestNames) and validate (suggestFieldName) to eliminate duplicated Levenshtein implementations
    suggest_test.go                18 tests (100% coverage) — Levenshtein distance, symmetry, TopN (close typo, exact match exclusion, no close matches, max results, sorted by distance, alphabetical tiebreaker, empty candidates, empty query), Best (close match, no match, exact match exclusion, multiple candidates, short field, alphabetical tiebreaker)
  shell/                           Shell shortcut file generation and management
    dialect.go                     ShellDialect interface (EvalWrapperFunc, InRepoCmd, AnywhereCmd, AllArgs, PositionalArg, FileHeader) — abstracts shell-specific syntax for multi-shell extensibility; BashDialect (bash/zsh concrete implementation); FishDialect (fish shell implementation — uses `function name ... end`, `set -l`, `$status`, `$argv`, `$argv[N]`, `env -C` for cd); DefaultDialect() → BashDialect
    shell.go                       GenerateShellFile(), GenerateShellFileWithDialect(), Install(), Uninstall(), ListInstalled(), PrimaryRCFile(), GenerateCompletionScript(), GenerateFishCompletionScript(), GenerateGlobalWrapper(), GenerateGlobalWrapperWithDialect(), FishFilePath(); with: shortcut codegen via buildWithArgs(dialect-aware); PI_PARENT_EVAL_FILE eval wrapper delegated to ShellDialect; pi-setup-<project> helper function; shell completion script generation; fish support: hasFishConfig() detects fish, installFishFiles() generates .fish shortcuts/wrapper/completion, ensureFishSourceLine()/removeFishSourceLine() manage config.fish source block; isInfraFile() identifies PI infrastructure files for list/uninstall filtering
    ...
    dialect_test.go                30 tests (BashDialect EvalWrapperFunc/InRepoCmd/AnywhereCmd/AllArgs/PositionalArg/FileHeader, FishDialect EvalWrapperFunc/InRepoCmd/AnywhereCmd/AllArgs/PositionalArg/FileHeader/QuotesSpaces/ComplexInnerCmd/PlugsIntoGeneration/GlobalWrapper, DefaultDialect type, custom mock dialect plugs into GenerateShellFileWithDialect and GenerateGlobalWrapperWithDialect, complex inner commands, paths with spaces, empty shortcuts, buildWithArgs unit tests including sort order and fish dialect)
    shell_test.go                  48 tests (includes completion script generation, lifecycle tests, fish install/uninstall/list integration, fish source line idempotency, fish file detection, infrastructure file filtering)
  ...
  validate/                        Project-wide static validation (registry-based, errors + warnings)
    validate.go                    Check interface, CheckFunc adapter, WarnCheck interface, WarnCheckFunc adapter, Context (Root/Config/Discovery), Result (Errors + Warnings), Runner (Register + RegisterWarn + Run + RunWithOpts), DefaultRunner() — pre-loads 12 error checks + 5 warning checks; individual check implementations: checkShortcutRefs, checkSetupRefs, checkRunStepRefs, checkFileReferences, checkShortcutInputs, checkSetupInputs, checkRunStepInputs, checkCircularDeps, checkConditions, checkUnknownFields, checkPiYamlUnknownFields, checkParentShellCapability; exported helpers: CheckWithInputs(), BuildRunGraph(), DetectCycles(), NormalizeCycleKey()
    warnings.go                    Warning checks (non-fatal): warnMissingDescription (automations without description:), warnUnusedAutomations (local automations not referenced by shortcut/setup/run:), warnShortcutShadowing (shortcuts shadowing shell builtins or common commands via shell.CheckShadowedNames), warnDuplicateSetupEntries (same run: target with identical if:/with: in setup: list — catches copy-paste mistakes), warnMissingInputDescription (local automations with inputs: that lack description: fields — hurts pi info and AI discoverability)
    unknown_fields.go              Unknown YAML field detection for automation files; checks top-level, step-level, install-block, and first: sub-step keys against known field sets; "did you mean?" suggestions via internal/suggest; skips builtins and package automations; knownAutomationKeys, knownStepKeys, knownInstallKeys field sets; suggestFieldName() delegates to suggest.Best(); fieldSuggestMaxDist() computes threshold (50% of field length, min 2)
    unknown_pi_yaml.go             Unknown YAML field detection for pi.yaml; checks top-level keys (project, shortcuts, setup, packages, runtimes) and runtimes: sub-keys (provision, manager) against known field sets; "did you mean?" suggestions via suggestFieldName() from unknown_fields.go
    validate_test.go               64 tests (runner lifecycle [empty, register, aggregation, counts, ordering], DefaultRunner check count + warn count (5), CheckFunc adapter, CheckWithInputs [no-with, all-valid, unknown, no-inputs, multiple-sorted], DetectCycles [no-cycles, direct, self-loop, three-node, diamond, multiple, disconnected], NormalizeCycleKey [rotation, self-loop, single, empty], individual checks [shortcut-refs valid/broken, setup-refs valid/broken, run-step-refs valid/broken, circular-deps cycle/no-cycle, conditions valid/unknown/automation-level, shortcut-inputs valid/unknown, setup-inputs valid/unknown/no-with/broken-ref, run-step-inputs valid/unknown/no-with/broken-ref/bash-skipped, file-refs inline/run-step/missing/existing, parent-shell-capability bash-ok/python-error/typescript-error/no-parent-shell-ok/inside-first-block], BuildRunGraph [basic, dedup])
    warnings_test.go               41 tests (WarnCheckFunc [name, run], runner warning integration [hidden by default, included with opts, aggregation, count, errors+warnings coexist], warnMissingDescription [with/without/mixed/skips-builtins], warnUnusedAutomations [shortcut/setup/run-step refs, unreferenced, multiple sorted, skips-builtins, all-referenced], warnShortcutShadowing [no-shadow, shell-builtin, common-command, multiple, no-shortcuts, pi.yaml prefix], warnDuplicateSetupEntries [no-duplicates, exact-duplicate, different-with, different-if, same-with, empty-setup, triple-duplicate, pi.yaml-prefix], warnMissingInputDescription [all-described, missing, mixed-inputs, no-inputs, skips-builtins, multiple-automations, skips-packages], RunWithOpts integration [clean project, all 5 warning types])
    unknown_fields_test.go         29 tests (valid file, valid multi-step, unknown top-level, top-level suggestion, unknown step-level, multiple unknowns, skips builtins, installer fields, unknown install field, first block sub-steps, missing file, empty file path, shorthand with all modifiers, step common typo, automation name in error, valid inputs, valid requires, installer with verify, parent shell and pipe, pipe_to, completely unknown no suggestion, invalid YAML, step and top-level mixed, suggestFieldName unit [exact/close/no match/multiple/short], Levenshtein unit [identical/empty/one diff/substitution])
...
pi run docker/up
  ...
  └─ Executor (internal/executor)
       ...
       --dry-run: prints steps without executing (evaluates conditions,
       resolves run: targets, shows first: match logic, installer lifecycle)
```
...
- Short-form tool name resolution via `tools.BuildShortNameMap()`: `python` → `pi:install-python`, `pi:go` → `pi:install-go`, `rust` → `pi:install-rust`, etc. (derived from `tools.Registry`)
- `tools.ToolResolutionHelp()` auto-generates the help text "Tool names are resolved automatically" section from the registry; deduplicates aliases, prefers canonical names matching `pi:install-<name>` suffix, sorts alphabetically
...
- `suggest` is a pure-logic package for Levenshtein edit distance and "did you mean?" suggestions — zero dependencies on other PI packages; used by `discovery` and `validate`
...
- `discovery` ties them together: walks the filesystem, calls `automation.Load()` for each file, builds the name→automation map; uses `refparser` to classify references in `Find()`/`FindWithAliases()`; `MergePackage()` discovers and merges package automations with alias tracking; uses `suggest` for "Did you mean?" error messages
...
- `validate` provides a registry-based validation system with `Check` interface, `Runner`, and `DefaultRunner()`; depends on `automation`, `config`, `discovery`, `conditions`, and `suggest` (no executor dependency — file-path detection and step metadata come from `automation.IsFilePath`, `automation.DefaultFileExtensions`, and `automation.StepTypeSupportsParentShell`)
...
- `SubprocessRunner` in `runners.go` is a config-driven runner that handles the full subprocess lifecycle: file-path resolution → inline-script handling (via `InlineArgs` or temp-file) → command execution → error wrapping. `SubprocessConfig` captures per-language differences (binary, inline arg format, temp file extension, file detection extension, not-found message, capability flags). All language-specific runners (`NewBashRunner`, `NewPythonRunner`, `NewTypeScriptRunner`) are thin config constructors. Adding a new language runner requires only a ~10-line config struct — no new types needed. `FileExt` on each runner determines which step values are treated as file references (e.g. `.sh` for bash, `.py` for python) — no global hardcoded extension list. `SupportsParentShell` flag indicates whether the runner can produce shell eval-able output (currently only bash).
...
- File-path resolution: `resolveFileStep()` in `helpers.go` combines `automation.IsFilePath(value, ext)` + `resolveScriptPath()` + existence check into a single call, eliminating per-runner duplication. `IsFilePath()` lives in the `automation` package (pure string check, no I/O) so both executor and validate can use it without circular dependencies. The extension is passed by the caller (from `SubprocessConfig.FileExt` at runtime, or `automation.DefaultFileExtensions()` at validation time), not hardcoded.
...
- Adding a new subprocess step type: (1) register the `StepType` constant in `automation.go` and add it to `validStepTypes`, (2) create a `New<Lang>Runner()` function in `runners.go` that returns a `&SubprocessRunner{Config: SubprocessConfig{...}}` with the language-specific config (including `FileExt` for file reference detection, `SupportsParentShell` for shell-eval capability), (3) register it in `NewDefaultRegistry()` in `runner_iface.go` — no new types, executor changes, or `helpers.go` changes needed. For non-subprocess step types (like `run:`), implement the `StepRunner` interface directly.
...
- Silent steps pass `io.Discard` for both `displayStdout` and `stderrW` to `execStep()` — no Executor state mutation needed
- Recursive `run:` step execution uses `runWithContext()` with explicit stdout/stdin parameters — no Executor state mutation during recursive calls
...
- `parent_shell` validity is determined by the runner's `SupportsParentShell` capability flag (only bash has it set); checked at `pi validate` time and as a runtime guard in the executor
...
- **Subprocess steps** (bash, python, typescript): `runStepCommand()` uses `exec.CommandContext` with `context.WithTimeout` when `Step.Timeout > 0`
- **Run steps**: `RunStepRunner.Run()` wraps the parent context with `context.WithTimeout` before calling `RunAutomation`; the deadline propagates through `runWithContext()` to all subprocess steps in the target automation; if the deadline is exceeded, the run step returns `ExitError{Code: 124}` — same semantics as subprocess timeouts
...
- Works with all step types: bash, python, typescript, run
- Compatible with all other step fields: `env:`, `dir:`, `silent:`, `if:`, `pipe: true`, `with:`
...
- `execInstallFirstBlock()` in `install.go` handles `first:` in install phases — delegates to `execStep()` for the matched sub-step
...
- `reqcheck.CheckRequirement()` (normal execution) and `reqcheck.CheckRequirementForDoctor()` (always-detect) share a `checkImpl()` core: PATH lookup via `LookPath()`, optional version detection and comparison.
...
- Two severity levels: errors (`Check` interface) and warnings (`WarnCheck` interface); warnings only run when `--warnings`/`-w` flag is passed
- `DefaultRunner()` pre-registers 11 error checks: shortcut-refs, setup-refs, run-step-refs, file-refs, shortcut-inputs, setup-inputs, run-step-inputs, circular-deps, conditions, unknown-fields, unknown-pi-yaml-fields
- `DefaultRunner()` pre-registers 3 warning checks: missing-description, unused-automations, shortcut-shadowing
- Adding a new error check = implement `validate.Check` interface + register with `Runner.Register()`
- Adding a new warning check = implement `validate.WarnCheck` interface + register with `Runner.RegisterWarn()`
...
- File reference checks: when a step value looks like a file path (detected by `automation.IsFilePath(value, ext)` — uses the per-step-type extension from `automation.DefaultFileExtensions()`, no newlines, no spaces), the path is resolved relative to the automation YAML file's directory and checked for existence on disk. Built-in automations are skipped (they use inline scripts only). Covers steps, `first:` block sub-steps, and install phase steps (including scalar install phases).
...
- Reports warnings with `⚠` prefixed lines to stderr (only with `--warnings` flag)
- Prints summary on success: `✓ Validated N automation(s), M shortcut(s), K setup entry(ies)` (appends warning count when warnings exist)
- Warnings are only shown when there are no errors (errors take precedence)
- Exit code 0 on success (even with warnings), 1 on validation errors (uses `*executor.ExitError{Code: 1}` for CLI exit handling)
...
### Value interpolation (`with:` and `env:`)
- `interpolateValue(v, inputEnv)` is the core resolver for three reference types:
...
- **`with:` values:** `InterpolateWith` callback on `RunContext` enables `RunStepRunner` to resolve references before passing them to the called automation
- **Step `env:` values:** `interpolateEnv()` resolves references per-step; the resolved map is set on `RunContext.ResolvedStepEnv` so `BuildStepEnv` uses it instead of the raw step env
- **Automation-level `env:` values:** resolved once at automation start (in `RunWithInputs`) and stored on `stepExecCtx.automationEnv`; `outputs.*` references resolve to empty at that point since no steps have run yet
- `interpolateEnv()` returns the original map when no values change (avoids allocation)
...
- `StatusKind` type with `StatusSuccess`, `StatusSuccessCached`, `StatusInProgress`, `StatusFailed`, `StatusWarning` constants — typed enum replaces raw icon strings for compile-time safety
- `InstallStatus(kind, name, status, version)` and `PackageFetch(kind, source, status, detail)` accept `StatusKind` for style dispatch: `StatusSuccessCached`→dim, `StatusSuccess`→bold green, `StatusFailed`→bold red, `StatusWarning`→yellow, `StatusInProgress`→plain
- `statusIcon(kind)` maps each `StatusKind` to its Unicode icon (✓, ✗, →, ↓, ⚠); `printStatusLine(kind, line)` routes to the correct style method — shared by `InstallStatus` and `PackageFetch`
...
- Used by: `Executor.printInstallStatus()` (with `display.StatusKind`), `Executor.RunWithInputs()` for step trace lines, `cli/discover.go` and `cli/add.go` package fetch status (with `display.StatusKind`), `cli/setup.go` headers, `cli/doctor.go` health output
...
- Key error types checked via `errors.As()`: `*executor.ExitError`, `*reqcheck.ValidationError`, `*config.DuplicatePackageError`, `*config.DuplicateSetupEntryError`, `*config.ReplacedSetupEntryError`, `*exec.ExitError`
...
- The `executor` package references `conditions.RuntimeEnv` directly on its `Executor.RuntimeEnv` field; no re-exports
...
tests/integration/
  ...
  step_timeout_integ_test.go      8 tests — step timeout: list, fast completes, slow exceeds (exit 124), mixed timed/untimed, info annotation, run step timeout exceeded (exit 124), run step timeout not exceeded, run step info shows timeout annotation
