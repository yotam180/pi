...
internal/
  ...
  executor/                        Step execution engine
    ...
    runner_iface.go                StepRunner interface, RunContext (step execution context with Ctx context.Context, WorkDir, RuntimePaths, InterpolateWith callback, RunAutomation callback (accepts context.Context), ResolvedAutomationEnv/ResolvedStepEnv override fields — env construction uses standalone BuildStepEnv with resolved env overrides), Registry (maps StepType→StepRunner), NewRegistry(), NewDefaultRegistry(), FileExtForStepType(), DefaultFileExtensions(), StepTypeSupportsParentShell()
    runners.go                     Step runner implementations: SubprocessRunner (config-driven, shared by bash/python/typescript), SubprocessConfig (Binary/BinaryFunc, InlineArgs, TempFileExt, FileExt, NotFoundMsg, SupportsParentShell, Language), NewBashRunner() (SupportsParentShell: true), NewPythonRunner(), NewTypeScriptRunner(), RunStepRunner (supports timeout: on run: steps via context.WithTimeout — timeout exceeded returns ExitError{Code: 124}); runStepCommand() shared command execution with timeout support (exec.CommandContext from RunContext.Ctx), calls BuildStepEnv(ctx.RuntimePaths, ...) directly; TimeoutExitCode (124); writeTempScript(), resolvePythonBin(), isCommandNotFound()
