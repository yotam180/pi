...
internal/
  ...
  executor/                        Step execution engine
    ...
    runner_iface.go                StepRunner interface, RunContext (step execution context with WorkDir, RuntimePaths, InterpolateWith callback — no BuildEnv callback; env construction uses standalone BuildStepEnv), Registry (maps StepType→StepRunner), NewRegistry(), NewDefaultRegistry(), FileExtForStepType(), DefaultFileExtensions(), StepTypeSupportsParentShell()
