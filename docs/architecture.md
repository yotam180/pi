...
internal/
  ...
  reqcheck/                        Runtime requirement checking (extracted from executor)
    reqcheck.go                    CheckResult, ValidationError, CheckRequirement(), CheckRequirementForDoctor(), checkImpl() (shared logic with alwaysDetectVersion flag); RuntimeCommand() — maps runtime names to CLI commands (python→python3, rust→rustc); DetectVersion(), detectVersionExec(), ExtractVersion(), CompareVersions(), parseVersionParts() — version detection and comparison utilities; FormatValidationError(), FormatRequirementLabel() — user-facing error formatting; InstallHintFor(), installHints map — install hint registry for common tools
  ...
  runtimes/                        Sandboxed runtime provisioning
    runtimes.go                    CmdRunner interface (Run/Output) — abstracts command execution for testability; execCmdRunner — default os/exec implementation; KnownRuntimes map (python/node/go/rust), Provisioner (with Runner field for CmdRunner injection), Provision(), provisionWithMise(), provisionDirect(), PrependToPath(), knownRuntimeList()
