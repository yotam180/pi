...
internal/
  ...
  shell/                           Shell shortcut file generation and management
    dialect.go                     ShellDialect interface (EvalWrapperFunc, InRepoCmd, AnywhereCmd, AllArgs, FileHeader) — abstracts shell-specific syntax for multi-shell extensibility; BashDialect (bash/zsh concrete implementation); DefaultDialect() → BashDialect; adding a new shell (e.g. fish) requires only implementing ShellDialect
    shell.go                       GenerateShellFile(), GenerateShellFileWithDialect(), Install(), Uninstall(), ListInstalled(), PrimaryRCFile(), GenerateCompletionScript(), GenerateGlobalWrapper(), GenerateGlobalWrapperWithDialect(); with: shortcut codegen via buildWithArgs(); PI_PARENT_EVAL_FILE eval wrapper delegated to ShellDialect; pi-setup-<project> helper function; shell completion script generation
    ...
    dialect_test.go                16 tests (BashDialect EvalWrapperFunc/InRepoCmd/AnywhereCmd/AllArgs/FileHeader, DefaultDialect type, custom mock dialect plugs into GenerateShellFileWithDialect and GenerateGlobalWrapperWithDialect, complex inner commands, paths with spaces, empty shortcuts, buildWithArgs unit tests including sort order)
