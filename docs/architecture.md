...
internal/
  ...
  executor/                        Step execution engine
    ...
    validate.go                    ValidateRequirements() (with provisioning fallback), tryProvision(); type aliases CheckResult = reqcheck.CheckResult, ValidationError = reqcheck.ValidationError; var delegations FormatValidationError, InstallHintFor, CheckRequirementForDoctor → reqcheck; unexported thin wrappers for backward compatibility with internal tests (checkRequirement, runtimeCommand, detectVersion, extractVersion, compareVersions, formatRequirementLabel, installHint)
