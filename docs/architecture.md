...
internal/
  ...
  executor/                        Step execution engine
    ...
    predicates.go                  RuntimeEnv (with ExecOutput field), DefaultRuntimeEnv(), ResolvePredicates(), ResolvePredicatesWithEnv(); resolves if: predicate names to booleans; ValidatePredicateName() — static validation that a predicate name matches known patterns without resolving runtime values; ValidateConditionExpr() — parses an if: expression and validates all predicate names; used by cli/validate.go for static analysis
    ...
    predicates_test.go             12 tests (+ subtests covering all predicate types) + ValidatePredicateName unit tests (25 valid/invalid cases) + ValidateConditionExpr unit tests (8 cases: simple/compound/complex/file-exists/empty/syntax-error/unknown/unknown-in-compound)
