...
- [ ] **Bug 2 — Scalar `InstallPhase` hardcodes Bash:** Not yet addressed as a separate task. The behavior is already implicitly documented in README examples (scalar install phases show shell commands). A future task could add explicit documentation or schema-level typing.
