...
Running `pi shell` writes shortcut functions into the user's shell config (`.zshrc` / `.bashrc`). When fish shell is detected (`~/.config/fish/config.fish` exists), fish-syntax shortcuts are also generated. Each shortcut is a shell function that:
...
When `~/.config/fish/config.fish` exists, `pi shell` also generates fish-syntax equivalents:

```fish
# Generated by: pi shell (fish)
function vpup
  set -l _pi_eval_file (mktemp)
  env -C /path/to/vyper-platform PI_PARENT_EVAL_FILE=$_pi_eval_file pi run docker/up $argv
...
Fish files are installed alongside bash/zsh files in `~/.pi/shell/`:
- `<project>.fish` — shortcut functions
- `_pi-wrapper.fish` — global `pi` wrapper
- `_pi-completion.fish` — completion setup

A source block is added to `~/.config/fish/config.fish` to load these files automatically.
